...
namespace YZSoft.Forms.Services
{
    ...
    public class DataBrowserServices : YZServiceHandler
    {
        ...
        /// <returns></returns>
        bool IsNewPrice(string VersionCode)
        {
            ...
            if(VersionCode.ToLower()=="no")//兼容前端请求
            {
                return false;
            }
        ...
        private JObject GetData(HttpContext context)
        {
            ...
            string CompanyCode = request.GetString("CompanyCode");
            int VersionType=request.GetInt32("VersionType");
            string VersionCode = request.GetString("VersionCode");
            string UserAccount = request.GetString("UserAccount");
 
            if(ds.TableName=="V_DetailMSG")
            {
                ds.ESB = "ESB:查询个性化施工老定额";
                ds.DataSource = "3";
            }
 
            this.PerformAfterBindFilter(filters, gridPageInfo);
            BPMDBParameterCollection @params = this.GetParameters(filters, supportAllSearchParams, runtimeParams, CompanyCode,VersionCode,VersionType,UserAccount);
        ...
        private BPMDBParameterCollection GetParameters(YZDSFilterCollection fixFilters, BPMObjectNameCollection supportAllSearchParams, YZClientParamCollection runtimeParams, string CompanyCode,string VersionCode,int VersionType,string UserAccount)
        {
            ...
            BPMDBParameter parameter1 = new BPMDBParameter("VersionType", typeof(Int32), VersionType);
            parameter1.ParameterCompareType = ParameterCompareType.NecessaryCondition | ParameterCompareType.Equ;
            @params.Add(parameter1);

            BPMDBParameter parameter2 = new BPMDBParameter("CompanyCode", typeof(String), CompanyCode);
            parameter2.ParameterCompareType = ParameterCompareType.NecessaryCondition | ParameterCompareType.Equ;
            @params.Add(parameter2);

            /* if (IsNewPrice(VersionCode) && VersionType==0)//新报价带版本号
             {

             }*/

            BPMDBParameter parameter3 = new BPMDBParameter("VersionCode", typeof(String), VersionCode);
            parameter3.ParameterCompareType = ParameterCompareType.NecessaryCondition | ParameterCompareType.Equ;
            @params.Add(parameter3);

            BPMDBParameter parameter4 = new BPMDBParameter("UserAccount", typeof(String), UserAccount);
            parameter4.ParameterCompareType = ParameterCompareType.NecessaryCondition | ParameterCompareType.Equ;
